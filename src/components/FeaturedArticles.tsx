...
const FeaturedArticles = () => {
  return <section id="ediciones" className="py-24 relative overflow-hidden\n">
      <div className="container px-0 mx-[130px]">
        ...
        margin: "-100px"
          ...
          {featuredArticles.map((article, index) => <Card key={article.id} className={`overflow-hidden hover:shadow-lg transition-all duration-300 ${index === 0 ? 'md:col-span-2' : ''}`}>
              <div className={`flex flex-col ${index === 0 ? 'md:flex-row' : ''} h-full`}>
                <div className={`${index === 0 ? 'md:w-1/2' : ''} relative h-56 overflow-hidden`}>
                  <motion.img src={article.imageSrc} alt={article.title} className="h-full w-full object-cover" whileHover={{
                scale: 1.05
              }} transition={{
                duration: 0.5
              }} />
                  <motion.div className="absolute top-3 left-3 bg-gradient-to-br from-nave-blue to-nave-lightblue text-white text-2xl px-3 py-2 rounded-md shadow-lg" whileHover={{
                scale: 1.1
              }} transition={{
                type: "spring",
                stiffness: 400,
                ...
                <div className={`p-6 flex flex-col ${index === 0 ? 'md:w-1/2' : ''} flex-grow`}>
                  ...
                  <CardFooter className="p-0 mt-auto">
                    <Button variant="ghost" className="group hover:bg-nave-blue/10 p-0" asChild>
                      <Link to={`/articulos/${article.id}`} className="flex items-center">
                        <span className="font-medium">Leer más</span>
                        <svg className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M5 12h14" />
